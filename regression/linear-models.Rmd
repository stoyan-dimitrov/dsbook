...
Why not? What exactly is the difference between the + and the : operators
...
Earlier, we ran the following linear model and looked at a summary of the results. 
...
Q: What null hypothesis is the second p-value (the one in the father row) testing?

A: β1=0, where β1 is the coefficient for the variable "father." 

Q6

Which R code(s) below would properly plot the predictions and confidence intervals for our linear model of sons’ heights? Select ALL that apply.
...
```{r}
fit_female <- female_heights %>% lm(mother ~ daughter, data = .)
...
```{r}
smry_fit_fml <- summary(fit_female)

pred_fit_fml <- predict(fit_female)
...
# each player's single rate and BB rate per season

bat_rate_99_01 <- Batting %>% filter(yearID %in% 1999:2001) %>%
    mutate(pa = AB + BB, singles = (H - X2B - X3B - HR)/pa, bb = BB/pa) %>%
    filter(pa >= 100) %>%
    group_by(yearID, playerID) %>%
    summarise(singles_season = sum(singles), bb_season = sum(bb))
```
...
bat_rate_avg_ssn_99_01 <- Batting %>% filter(yearID %in% 1999:2001) %>%
    mutate(pa = AB + BB, singles = (H - X2B - X3B - HR)/pa, bb = BB/pa) %>%
    filter(pa >= 100) %>%
    group_by(playerID) %>%
    summarise(singles_mean = mean(singles), bb_mean = mean(bb))
```

How many players had a single rate mean_singles of greater than 0.2 per plate appearance over 1999-2001?

```{r}
bat_rate_avg_ssn_99_01 %>%
    filter(singles_mean > 0.2) %>%
    nrow()
```
...
```{r}
bat_02_vs_99_01 <- inner_join(bat_02, bat_rate_avg_ssn_99_01, by = c("playerID" = "playerID"))
```
...
```{r}
bat_02_vs_99_01 %>%
    summarise(r = cor(singles, singles_mean))
```
...
```{r}
bat_02_vs_99_01 %>%
    ggplot(aes(singles_mean, singles)) +
    geom_point(alpha = 0.5) +
...
```{r}
bat_02_vs_99_01 %>% lm(singles ~ singles_mean, data = .)
```


```{r}
bat_02_vs_99_01 %>% lm(bb ~ bb_mean, data = .)
```
...
### Assessment: Linear regression in the tidyverse
...
```{r}
dat02 <- Teams %>% filter(yearID %in% 1961:2001) %>%
  mutate(HR = HR/G,
         R = R/G) %>%
  select(lgID, HR, BB, R) 
```
...
Same as the above but using count instead of group_by and summarise
...
galton %>%
    ...
    filter(term == "parentHeight") %>%
    group_by(pair) %>%
    summarise(conf.low < 0 && conf.high > 0)
```
...
players <- Batting %>% filter(yearID %in% 1997:2001) %>% 
  group_by(playerID) %>%
  mutate(PA = BB + AB) %>%
  summarize(G = sum(PA)/pa_per_game, ## the sum of all the PA in the 1997-2001 period and divided by the mean PA for 2002 period so we know how many "complete" games a player has had during 1997-2001 period (with "refference" of the 2002 period)
    BB = sum(BB)/G,
...
## Assessment:Case study: Moneyball (continued)
...
data.frame(playerID = c("teamA", "teamB"), BB = c(2, 1), singles = c(4, 6), doubles = c(1, 2), triples = c(0, 1), HR = c(1, 0)) %>%
    mutate(r_hat = predict(fit, newdata = .))
```
...
Interpret the p-values for the estimates using a cutoff of 0.05. Which of the following is the correct interpretation?
...
fit <- Teams_small %>%
  ...
  summarise(W = sum(W), avg_attendance = (mean(avg_attendance))) %>%
  ungroup() %>%
...
Stratify Teams_small by wins: divide number of wins by 10 and then round to the nearest integer. Keep only strata 5 through 10, which have 20 or more data points. (The last part for the data points is not clear and for that reason it has not been included in the following code, nevertheless the result was accepted as correct.)


```{r}
Teams_small %>%
    mutate(W_str = round(W/10, 0)) %>%
    filter(W_str %in% 5:10)
```
...
data.frame(yearID = c(1960, 2002), RpG = c(5, 5), HRpG = c(1.2, 1.2), W = c(80, 80)) %>%
    mutate(r_hat = predict(fit_ts, newdata = .))
```
...
Teams %>%
    ...
    mutate(avg_attendance_hat = predict(fit_ts, newdata = .)) %>%
    summarize(cor(avg_attendance, avg_attendance_hat))
```
